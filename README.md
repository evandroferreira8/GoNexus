...
Para adicionar uma nova ferramenta ao menu, crie o novo arquivo HTML e acrescente uma entrada ao array `pages` em `nav.js`. As páginas que carregam `nav.js` passarão a exibir a nova opção automaticamente.
...
O GO Nexus é um projeto independente e não oficial criado para a comunidade de Pokémon GO. Não possui afiliação, patrocínio ou vínculo oficial com os titulares das marcas e propriedades intelectuais relacionadas ao jogo.
