(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:640px){.gnx-ad-wrap{width:min(100% - 18px,1120px);margin:14px auto}.gnx-ad-box{min-height:82px}.gnx-ad-wrap ins.adsbygoogle{min-height:82px}.gnx-sponsor-card{grid-template-columns:1fr;min-height:0;padding:14px}.gnx-sponsor-card b{width:max-content}}
    `;
  ...
  function insertAdSlots() {
    if (document.querySelector(".gnx-ad-wrap")) return;
    addStyles();
    ...
    if (header?.parentNode) header.insertAdjacentElement("afterend", configured && topConfig ? buildAd(topConfig) : buildSponsorFallback("after-header"));

    if (main) {
      const sections = [...main.children].filter((child) => !child.matches("script, style"));
      const anchor = sections[Math.min(2, sections.length - 1)];
      const slot = configured && midConfig ? buildAd(midConfig) : buildSponsorFallback("mid-content");
